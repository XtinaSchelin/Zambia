	...
	<xsl:template match="doc/moderator">
		<xsl:choose>
			<xsl:when test="@changecode='AM'">
				...
				<li>Set <xsl:value-of select="$addedModerator"/>
 (				<xsl:value-of select="$addedModeratorBadgeid"/>
) as moderator.</li>
		</xsl:when>
		<xsl:when test="@changecode='RM'">
			...
			<li>Remove <xsl:value-of select="$removedModerator"/>
 (			<xsl:value-of select="$removedModeratorBadgeid"/>
) as moderator.</li>
	</xsl:when>
	<xsl:when test="@changecode='CM'">
		...
		<xsl:variable name="removedModeratorBadgeid" select="@frommoderator" />
		<xsl:variable name="removedModerator" select="/doc/query[@queryName='participants']/row[@badgeid=$removedModeratorBadgeid]/@pubsname" />
		<li>Change moderator from <xsl:value-of select="$removedModerator"/>
 (		<xsl:value-of select="$removedModeratorBadgeid"/>
) to <xsl:value-of select="$addedModerator"/>
 (	<xsl:value-of select="$addedModeratorBadgeid"/>
).</li>
</xsl:when>
</xsl:choose>
...
<li>Add <xsl:value-of select="$addedParticipant"/>
...
<li>Remove <xsl:value-of select="$removedParticipant"/>
 (<xsl:value-of select="$badgeid"/>
) from session.</li>
</xsl:template>
