...
SELECT POS.badgeid AS posbadgeid,
  ...
  (SELECT DISTINCT badgeid,
   ...
   FROM
     (SELECT badgeid,
      ...
      WHERE sessionid = $selsessionid
      UNION SELECT badgeid,
                   sessionid
      FROM ParticipantSessionInterest
      WHERE sessionid=$selsessionid) AS R2) AS R USING (badgeid)
LEFT JOIN ParticipantSessionInterest AS PSI ON R.badgeid = PSI.badgeid
AND R.sessionid = PSI.sessionid
LEFT JOIN ParticipantOnSession AS POS ON R.badgeid = POS.badgeid
AND R.sessionid = POS.sessionid
LEFT JOIN ParticipantHasRole AS PHR ON P.badgeid = PHR.badgeid
AND PHR.roleid = 10 /* moderator */
WHERE POS.sessionid = $selsessionid
  OR POS.sessionid IS NULL
ORDER BY attending DESC,
         moderator DESC,
         IFNULL(POS.badgeid, "~") ASC, rank ASC,
                                       P.pubsname ASC;
EOD;
