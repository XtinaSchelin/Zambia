...
EOD;
   ...
   (SELECT createdts AS TIMESTAMP
	FROM ParticipantOnSessionHistory
	WHERE sessionid = $selsessionid)
 UNION
   (SELECT inactivatedts AS TIMESTAMP
	FROM ParticipantOnSessionHistory
	WHERE sessionid = $selsessionid)
 UNION
   (SELECT TIMESTAMP
...
SELECT COALESCE(POS.moderator, 0) AS moderator,
	   P.badgeid,
	   P.pubsname
...
FROM ParticipantOnSessionHistory POSH
...
WHERE POSH.sessionid = $selsessionid;
EOD;
...
SELECT SEH.badgeid,
	   ...
	   SS.statusname
FROM SessionEditHistory SEH
JOIN SessionEditCodes SEC USING (sessioneditcode)
JOIN SessionStatuses SS USING (statusid)
WHERE SEH.sessionid = $selsessionid;
EOD;
}
