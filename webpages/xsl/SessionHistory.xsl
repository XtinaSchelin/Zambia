...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="doc/query[@queryName='chooseSession']/row">
		<option value="{@sessionid}">
			<xsl:if test="@sessionid = $selsessionid">
				<xsl:attribute name="selected">selected</xsl:attribute>
			</xsl:if>
 ...
 (				<xsl:value-of select="@badgeid" />
					...
					<span class="za-sessionHistory-moderator">moderator</span>
				</xsl:if>
			</span>
		</div>
	</xsl:template>
						...
						<div class="span9 offset3">
							<xsl:choose>
								<xsl:when test="count($createParticipantRow) > 0">
									<xsl:value-of select="$createParticipantRow/@crpubsname"/>
 (									<xsl:value-of select="$createParticipantRow/@createdbybadgeid" />
)
									<xsl:value-of select="$createParticipantRow/@createdtsformat" />
								</xsl:when>
								<xsl:when test="count($inactivateParticipantRow) > 0">
									<xsl:value-of select="$inactivateParticipantRow/@inactpubsname"/>
 (									<xsl:value-of select="$inactivateParticipantRow/@inactivatedbybadgeid" />
)
									<xsl:value-of select="$inactivateParticipantRow/@inactivatedtsformat" />
								</xsl:when>
								<xsl:when test="count($editSessionRow) > 0">
									<xsl:value-of select="$editSessionRow/@name"/>
 (									<xsl:value-of select="$editSessionRow/@badgeid" />
)
				...
				<span class="span5 offset1">
					<xsl:choose>
						<xsl:when test="count($addModeratorRow) > 0 and count($deleteModeratorRow) > 0">
							Change moderator from <xsl:value-of select="$deleteModeratorRow/@pubsname"/>
 (						<xsl:value-of select="$deleteModeratorRow/@badgeid" />
)
							to <xsl:value-of select="$addModeratorRow/@pubsname"/>
 (					<xsl:value-of select="$addModeratorRow/@badgeid" />
).
				</xsl:when>
				<xsl:when test="count($addModeratorRow) > 0">
							Assign <xsl:value-of select="$addModeratorRow/@pubsname"/>
 (				<xsl:value-of select="$addModeratorRow/@badgeid" />
) as moderator.
			</xsl:when>
			<xsl:otherwise>
							Remove <xsl:value-of select="$deleteModeratorRow/@pubsname"/>
 (			<xsl:value-of select="$deleteModeratorRow/@badgeid" />
) from moderator.
		</xsl:otherwise>
	</xsl:choose>
...
<div class="row-fluid">
<span class="span5 offset1">
					Add <xsl:value-of select="@pubsname"/>
 (<xsl:value-of select="$badgeid" />
) to panel.
</span>
</div>
...
<div class="row-fluid">
<span class="span5 offset1">
					Remove <xsl:value-of select="@pubsname"/>
 (<xsl:value-of select="$badgeid" />
) from panel.
</span>
</div>
