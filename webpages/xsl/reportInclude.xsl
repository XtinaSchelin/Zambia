...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
 ...
 Min
            ...
            <xsl:when test="$durationmin='00'">
                <xsl:value-of select="@durationhrs" />
 Hr
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="$durationhrs" />
 Hr <xsl:value-of select="$durationmin" />
 Min
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="showSessionid">
        ...
        <a href="StaffAssignParticipants.php?selsess={$sessionid}" title="Edit session participants">
    ...
    <xsl:template name="showSessionTitle">
        ...
        <a href="EditSession.php?id={$sessionid}" title="Edit session">
    ...
    <xsl:template name="showRoomName">
        ...
        <a href="MaintainRoomSched.php?selroom={$roomid}" title="Maintain room schedule">
    ...
    <xsl:template name="showSessionidWithTitle">
        <xsl:param name="sessionid" />
        <xsl:param name="title" />
        <a href="EditSession.php?id={$sessionid}" title="{$title}">
            <xsl:value-of select="$sessionid" />
        </a>
    </xsl:template>
